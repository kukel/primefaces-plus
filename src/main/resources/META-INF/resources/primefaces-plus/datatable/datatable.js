/**
 * PrimeFaces DataTable2 Widget
 */
PrimeFaces.widget.TreeDataTable = PrimeFaces.widget.DeferredWidget.extend({
	...
	this.cfg.paginator.paginate = function(newState) {
	    _self.paginate(newState);
       	...
       	this.paginator = new PrimeFaces.widget.Paginator2(this.cfg.paginator);
    },
    ...
    bindExpansionEvents: function() {
		...
		$(document)
			.off(
				'click.datatable2',
				this.jqId
					+ ' tbody.ui-datatable-data tr td span.ui-treetable-toggler')
			...
			.on(
				...
				function(e) {
				    e.stopPropagation();
				    var toggler = $(this), node = toggler
					    .parents('tr:first');

				    if (toggler
					    .hasClass('ui-icon-triangle-1-e'))
					_self.expandNode(e, node, _self);
				    else {
					_self.collapseNode(e, node, _self);
				    }
				});
	    },

	    expandNode : function(e, node, _self) {
		// console.log('Expand');
		node.find('.ui-treetable-toggler:first').addClass(
			'ui-icon-triangle-1-s').removeClass(
			'ui-icon-triangle-1-e');
		node.attr('aria-expanded', true);
		_self.expandChildren(node, _self);
		_self.resizeHeight();
		_self.applyRowClasses();
	    },

	    expandChildren : function(node, _self) {
		sibblings = node.siblings('[id^="' + node.attr('id') + '_"]');
		sibblings.each(function(i, e) {
		    var childId = e.id.substr(node.attr('id').length);
		    console.log(i, node.attr('id'), childId);
		    if (childId.length == 2) {
			if (($(e).attr('aria-hidden') == 'true')
				|| ($(e).attr('aria-hidden') === undefined)) {
			    console.log('showing ', childId);
			    $(e).removeAttr('aria-hidden');
			    $(e).show();
			}
			expanded = $(e).attr('aria-expanded');
			if (expanded === undefined) {
			    console.log('Has no children...', childId);
			} else {
			    var processChildren = expanded == "true";
			    if (processChildren === true) {
				_self.expandChildren($(e), _self);
			    } else {
				console.log('ignoring children of ', childId);
			    }
			}

		    } else {
			console.log('ignoring ', childId);
		    }

		});
	    },

	    collapseNode : function(e, node, _self) {
		node.find('.ui-treetable-toggler:first').addClass(
	...
	options.oncomplete = function() {
	    ...
	    $this.paginator.cfg.page = newState.page;
           ...
           $this.paginator.updateTotalRecords(this.args.totalRecords);
	};
    ...
    applyRowClasses : function() {
	// console.debug('applying row classes...');
	$(this.jqId + " .ui-datatable-scrollable-body tr:not([aria-hidden='true'])").each(
	    function(k, e) {
		$(e).removeClass('ui-datatable-even')
		    .removeClass('ui-datatable-odd')
		    .addClass(k % 2 == 0 ? 'ui-datatable-even' : 'ui-datatable-odd');
	    }
	);
    },
	...
	this.scrollHeader = $(_self + ' .ui-datatable-scrollable-header');
	this.scrollHeaderBox = $(_self + ' .ui-datatable-scrollable-header-box');
	this.scrollBody = $(_self + ' .ui-datatable-scrollable-body');
	this.scrollFooter = $(_self + ' .ui-datatable-scrollable-footer');
	this.paginatorBottom = $(_self + ' .ui-paginator-bottom');
		
	var unusableSpace = 0;
	if (this.scrollHeader != undefined) {
	    unusableSpace += this.scrollHeader.height();
	}
	if (this.scrollFooter != undefined) {
	    unusableSpace += this.scrollFooter.height();
	}
	if (this.paginatorBottom != undefined) {
	    unusableSpace += this.paginatorBottom.height();
	}

	var filler = $(_self + " .ui-datatable-scrollable-body table")[1];
	if (filler != undefined) {
	    $(filler).find('tbody').empty();
	}
	var newHeight = (this.jq.parent().height() - this.jq.position().top - unusableSpace);
	this.scrollBody.css({'height' : newHeight + 'px'});

	if (this.scrollBody[0].offsetHeight < this.scrollBody[0].scrollHeight) {
	    // your element have overflow
	    // implement detecting scrollbar size
	    this.scrollHeaderBox.css({
		'margin-right' : '15px'
	    });
	    this.scrollBody.css({
		'overflow' : 'auto'
	    ...
	    console.log("Filler", filler);
            if (filler === undefined) {
		filler = $('<table id="filler" style="table-layout:fixed; width: 100%; margin-top: -1px"><tbody class="ui-datatable-data ui-widget-content"></tbody></table>');
		$(_self + ' .ui-datatable-scrollable-body').append(filler);
		filler = $(this.scrollBody).find('table')[1];
		var colgroup = $(_self + "_data ").parent().find('colgroup');
		var colgroupclone = $(colgroup).clone(true);
		$(filler).append(colgroupclone);
	    }

	    var lastRow = $(_self + "_data ").find('tr').last();
	    var clone = $(lastRow).clone(true);
	    $(clone).find('td').html('&nbsp;');
	    // Since copying the last row MIGHT be an invisible one...
	    // explicitly set it to show... otherwise an endless loop
	    $(clone).show().removeAttr('data-rk').removeAttr('data-ri');
	    $(clone).on('click.datatable2', function(e) {
 	        // No idea yet how to remove the click event... so just
	        // adding one and preventing 'bubble up'
	        e.stopPropagation();
	    });
	    var rowsHeight = $(this.scrollBody).find('table')[0].offsetHeight;
	    var lastFillerRow;
	    console.log(clone.height(), (rowsHeight + filler.offsetHeight),
	    this.scrollBody[0].offsetHeight);
	    while ((rowsHeight + filler.offsetHeight) <= this.scrollBody[0].offsetHeight) {
		lastFillerRow = $(filler).find('tbody').find('tr').last();
		if (lastFillerRow.size() == 0) {
		    clone.removeClass('ui-state-highlight')
		    clone.toggleClass('ui-datatable-odd').toggleClass('ui-datatable-even');
		    $(filler).find('tbody').append(clone);
		} else {
		    clone = $(lastFillerRow).clone(true);
		    clone.removeClass('ui-state-highlight')
		    clone.toggleClass('ui-datatable-odd').toggleClass('ui-datatable-even');
		    $(clone).insertAfter(lastFillerRow);
		}
		console.log(lastFillerRow.height(),filler.offsetHeight,
		this.scrollBody[0].offsetHeight,
		(rowsHeight + filler.offsetHeight));
		if ((rowsHeight + filler.offsetHeight) > this.scrollBody[0].offsetHeight) {
		    console.log(lastFillerRow.height(), (rowsHeight + filler.offsetHeight),this.scrollBody[0].offsetHeight);
		    console.log(lastFillerRow.height() - ((rowsHeight + filler.offsetHeight) - this.scrollBody[0].offsetHeight));
		}
	    }
	}
	var _baseHeader = _self + ' .ui-datatable-scrollable-header-box table';
	// console.log(_self, $($(_self + '.ui-datatable-scrollable-header-box table')), $(_self + '.ui-datatable-scrollable-body table').width());
	$(_self + '_data .ui-datatable-scrollable-body table').each(
		function(j, e) {
		    $($(_baseHeader)).width($(e).width());
		});

	//var _baseBody = _self + ' .ui-datatable-scrollable-header-box table';
	//$(_self + '_data .ui-datatable-scrollable-body table')
	//    .find('tr')
	//    .first()
        //    .children()
	//   .each(function(i, e) {
		// console.log(i,
		// $($($(_baseBody).children()[0]).children()[1]).children()[i],
		// $(e).width());
		// $($($($(_baseBody).children()[0]).children()[0]).children()[i]).width($(e).width());
	        //console.log(
		//	$(e).width(), $($($($(_baseBody).children()[0]).children()[0]).children()[i]).width());
	//});
