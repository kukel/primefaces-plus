...
import org.primefaces.plus.component.datatable.feature.DataTableFeature;
import org.primefaces.plus.component.datatable.feature.DataTableFeatureKey;
import org.primefaces.plus.component.datatable.feature.SortFeature;
import org.primefaces.plus.component.datatable.feature.TreeFeature;
...
public class DataTableRenderer extends DataRenderer {
    ...
    protected void preEncode(FacesContext context, DataTable table) {
    	// Lazy is not officially supported (yet?)
        if(table.isLazy()) {
	...
	protected void encodeScript(FacesContext context, DataTable table) throws IOException{
        ...
        wb.initWithDomReady("TreeDataTable", table.resolveWidgetVar(), clientId);
        ...
        // scrollHeight of 0 means 'auto height', kind of like 100%
        if("auto".equalsIgnoreCase(table.getScrollHeight())) {
          ...
          sb.append("  PF('" + table.resolveWidgetVar()+"').resizeHeight();\n");
          sb.append("  " + table.resolveWidgetVar()+".applyRowClasses();\n");
          sb.append("});\n");
          
          sb.append("\n");
          sb.append("$(window).resize(function(){\n");
          sb.append("  clearTimeout(resizeDelay" + table.resolveWidgetVar() +");\n");
          sb.append("  var resizeDelay" + table.resolveWidgetVar() +" = window.setTimeout(\n");
          sb.append("    function() {\n");
          sb.append("      PF('" + table.resolveWidgetVar()+"').resizeHeight();\n");
          sb.append("    }, 300\n");
          sb.append("  );\n");
    ...
    protected void encodeScrollBody(FacesContext context, DataTable table, String tableStyle, String tableStyleClass) throws IOException {
        ...
        boolean autoHeight = "auto".equalsIgnoreCase(scrollHeight);
        writer.startElement("div", null);
        writer.writeAttribute("class", DataTable.SCROLLABLE_BODY_CLASS + (autoHeight ? "" : " " + DataTable.AUTO_HEIGHT_CLASS) , null);
        if(scrollHeight != null && scrollHeight.indexOf("%") == -1 && !autoHeight) {
            writer.writeAttribute("style", "height:" + scrollHeight + (scrollHeight.endsWith("px") ? "" : "px"), null);
        }
        writer.startElement("table", null);
        writer.writeAttribute("role", "grid", null);
    ...
    public void encodeTbody(FacesContext context, DataTable table, boolean dataOnly) throws IOException {
        ...
        // This is the amount of records that are not 'toplevel'
        int childRecordsCount = table.getChildRecordsCount();
        ...
        int row = 0;
        if(hasData) {
            //for(int i = first ; i < (first + rowCountToRender + childRecordsCount); i++) {
            ...
            if (table.isTree()) {
	        	...
	        	for(int i = 0; i < (rowCountToRender + childRecordsCount); i++) {
	                if(subTable != null) {
	                    ...
	                    // i is the number of rows that was totally encoded up to now
	                    // so up to his row and its children
	                    i = encodeRootRow(context, table, clientId, i, rowIndexVar, row);
        		...
        		encodeRows(context, table, first, first+rowCount);
        	}
        }
        else {
            //Empty message
    ...
    private void encodeRow(FacesContext context, DataTable table, String clientId, int i) {
    	throw new RuntimeException("Implement 'encodeRow'");
	}
	...
	protected void encodeFrozenRows(FacesContext context, DataTable table) throws IOException {    	
        ...
        for(Iterator<? extends Object> it = frozenRows.iterator(); it.hasNext();) {
            ...
            // Frozen rows is not officially supported in the treedatatable 
    ...
    public int encodeRow(FacesContext context, DataTable table, String clientId, int rowIndex, String rowIndexVar) throws IOException {
    	return encodeRow(context, table, clientId, null, rowIndex, clientId + "_node_0", null);
    }

    public int encodeRootRow(FacesContext context, DataTable table, String clientId, int rowIndex, String rowIndexVar, int row) throws IOException {
    	return encodeRow(context, table, clientId, null, rowIndex, clientId + "_node_" + row, null);
    }
    
    public int encodeRow(FacesContext context, DataTable table, String clientId, Object child, int rowIndex, String nodeId, Boolean parentExpanded) throws IOException {
        ResponseWriter writer = context.getResponseWriter();
        ...
        if(selectionEnabled) {
            ...
            if(rowKey == null) {
            	if(child != null) {
            		rowKey = table.getRowKeyFromModel(child);
            	} else {
        ...
        //writer.writeAttribute("aria-expanded", String.valueOf(table.isRowHasChildren()), null);
        
        boolean expanded = false;
        
        if (parentExpanded != null && !parentExpanded) {
        	writer.writeAttribute("aria-hidden", String.valueOf(true), null);
        	writer.writeAttribute("style", "display: none", null);
        } else {
        	if (table.rowHasChildren()) {
        ...
        for(UIColumn column : table.getColumns()) {
            ...
            if(column instanceof Column) {
                encodeCell(context, table, column, clientId, selected, firstColumn, nodeId, expanded);
            }
            else if(column instanceof DynamicColumn) {
                DynamicColumn dynamicColumn = (DynamicColumn) column;
                dynamicColumn.applyModel();

                encodeCell(context, table, dynamicColumn, null, false, firstColumn, nodeId, expanded);
            }
            
        	if (firstColumn == true && column.isRendered()) {
                firstColumn = false;
        	}
        }
        ...
        if (table.rowHasChildren()) {
        	Collection children = (Collection)table.getRowChildren();
        	int j=0;
        	for (Object sibbling : children) {
        		rowIndex++;
        		...
        		rowIndex = encodeRow(context, table, clientId, sibbling, rowIndex, nodeId+SEPARATOR+j, expanded);
        		j++;
			}
	...
	protected void encodeCell(FacesContext context, DataTable table, UIColumn column, String clientId, boolean selected, boolean firstColumn, String nodeId, boolean expanded) throws IOException {
        if(!column.isRendered()) {
            return;
        }
        ...
        int depth = nodeId.split(SEPARATOR).length - 3;
        rowToggler(table, firstColumn, depth, writer, expanded);
	...
	private void rowToggler(DataTable table, boolean firstColumn, int depth, ResponseWriter writer, boolean expanded) throws IOException {
		...
		if (firstColumn) {
	        ...
	        String cellStyle;
	        int paddingLeft = 0;
	        int marginLeft =  10*depth ;
	        if (table.rowHasChildren() == false) {
	        	paddingLeft = 15;
	        } else {
	        	paddingLeft = 0;
	        }
	        cellStyle = "margin-left:" + marginLeft + "px; padding-left:" + paddingLeft + "px; display: inline-block";
	//        if(scrollable) {
	...
	//        }
	        ...
	        if(table.rowHasChildren() == true) {
	        	String icon = expanded ? TreeTable.COLLAPSE_ICON : TreeTable.EXPAND_ICON ;
