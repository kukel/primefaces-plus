...
(function($) {
    ...
    $.fn.Ribbon = function(ribbonSettings) {
        var settings = $.extend({ theme: 'descartes' }, ribbonSettings || {});
        ...
        $('head').append('<link href="/mercurius/ribbon/themes/' + settings.theme + '/ribbon.css" rel="stylesheet" type="text/css" />" />');
        ...
        function SetupMenu(settings) {
            ...
            $('.ribbon-list div').parents().click(function(e) {
            	var parent = $('.ribbon-list div ul:visible').parent();
            	
            	if (parent != undefined && parent.size() != 0) {
            		var outsideX = e.pageX < parent[0].offset().left || e.pageX > parent[0].offset().left + parent[0].width();
                	var outsideY = e.pageY < parent[0].offset().top || e.pageY > parent[0].offset().top + parent[0].height();
        ...
        $('.ribbon').parents().click(function(e) {
        	...
        	var first = $('.orb ul:first');
        	var firstImg = $('.orb ul:first img:first');
        	if (first != undefined && first.size() != 0 && firstImg != undefined && first.size() != 0) {
        		var outsideX = e.pageX < first[0].offset().left || e.pageX > first[0].offset().left + first[0].width();
        		var outsideY = e.pageY < $('.orb ul:first img:first')[0].offset().top || e.pageY > first[0].offset().top + first[0].height();
       			if (outsideX || outsideY)
       				$('.orb ul').fadeOut('fast');
       		}
        });
        ...
        $('.orb li:first ul:first').prepend('<img src="/mercurius/ribbon/themes/' + settings.theme + '/images/menu_top.png" style="margin-left: -10px; margin-top: -22px;" />');
        $('.orb li:first ul:first').append('<img src="/mercurius/ribbon/themes/' + settings.theme + '/images/menu_bottom.png" style="margin-left: -10px; margin-bottom: -22px;" />');

        $('.ribbon-list div').each(function() { if ($(this).children('ul').length > 0) { $(this).append('<img src="/mercurius/ribbon/themes/' + settings.theme + '/images/arrow_down.png" style="float: right; margin-top: 5px;" />') } });
